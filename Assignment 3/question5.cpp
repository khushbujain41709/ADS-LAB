...
Node* unionLinkedLists(Node* &head1, Node* &head2){
    if(head1 == NULL) return head2;
    if(head2 == NULL) return head1;
    Node* unionList = NULL;
    unordered_set<int> visited;
    // Traversing linkedlist 1
    Node* temp = head1;
    while(temp != NULL){
        if(not visited.count(temp->val)){
            visited.insert(temp->val);
            unionList = insertAtTail(unionList, temp->val);
        }
        temp = temp->next;
    }
    // Traversing linkedlist 2
    temp = head2;
    while(temp != NULL){
        if(not visited.count(temp->val)){
            visited.insert(temp->val);
...
int main(){
    ...
    display(ll2);
    Node* ll3 = unionLinkedLists(ll1, ll2);
    display(ll3);
}